...
format:
  revealjs:
    output-file: chapter3normal.html
  html: 
    output-file: chapter3normalPrintable.html
---
...
The [normal]{.definition} (also known as 'Gaussian') is $X\sim N(\mu_X,\sigma_X^2)$:

-   $\mu_X$ is the expected value ($E[X]$)
-   $\sigma_X^2$ is the variance of ($Var[X]$)
...
We will always write $X\sim N(\mu_X,\sigma_X^2)$ for the normal
...
## Computing values (quantiles) {.scrollable}
...
xMin = mu - sqrt(sigmaSq)*4
xMax = mu + sqrt(sigmaSq)*4
print(ggplot(data.frame(x = c(xMin, xMax)), aes(x = x)) + 
    stat_function(fun = dnorm, args = list(mean = mu, sd = sqrt(sigmaSq))) + 
    stat_function(fun = funcShaded, args = list(lower_bound = x), 
                  geom = "area", fill = 'lightblue', alpha = .5) +
    scale_x_continuous(name = lab) + scale_y_continuous(name = "f(x)"))
...
$$
f_Z(z)=\frac1{\sqrt{2\pi}}e^{-z^2/2},\,\,-\infty<z<\infty.
$$
...
[(For a value $x$ from a distribution with mean $\mu_X$ and variance $\sigma_X^2$)]{.subscript}

[Question:]{.smallCap} Which of the two values from two different distributions is more [unusual]{.emphasis}?


[Answer:]{.smallCap} Whichever z-score has larger [magnitude]{.emphasis} 
...
| record   |  $x = 9.63$ |  $x = 10.61$ |

: Table of Olympic Sprinting times (s)
...
The z-score for men is $\text{z-score}_M$ = `r round(zScoreM,3)` while for women it is $\text{z-score}_W$ = `r round(zScoreW,3)`

:::

## Z-scores: Another application {.scrollable}

Another use of [z-scores]{.definition}: what value $x$ would be equally as unusual?
...
[Example:]{.example} What time for a men's sprinter would be equivalent to the female record?
...
$$
\text{z-score}_W = \frac{10.61 - \mu_W}{\sigma_W} = \frac{10.61 - 10.83}{\sqrt{.0049}}
$$
= `r round(zScoreW,3)`

To find the equivalent time, we need to [unstandardize]{.definition
}
$$
\text{z-score}_W*\sigma_M  + \mu_M = x
$$

Here, use
 
-   z-score from the female sprinters
-   the $\mu_M$ and $\sigma_M$ from the male sprinters ($\mu_M = 9.85$, $\sigma_M^2 = .0057$)
...
The male record would need to be `r round(x,3)` seconds to be as unusual as the female record.

:::

# Empirical Rule: A quick guide to normal probabilities
...
[probability = 0.68]{.lightbluetext}
:::
...
outPlot
```
[probability = 0.95]{.lightgreentext}
:::
...
outPlot
```
[probability = 0.997]{.lightredtext}
:::
::::::
...
## Empirical rule: Pulsar example {.scrollable}


```{r}
#| label: normalEmpRuleExample
#| echo: false
#| fig-asp: .3
#| out-width: "210%"
n     = 29
...
In looking at a sample of `r n` pulsar stars, they had periodicity:

```{r}
#| label: normalEmpRuleExample2
#| echo: false
#| fig-asp: .3
#| out-width: "210%"

plot(x,rep(0,n),pch=3,xlab='periodicity',ylab='',xlim=c(mu-3*sigma,mu+3*sigma),yaxt='n',xaxt='n')
axis(1,at=seq(round(mu-3*sigma,0),round(mu+3*sigma,0),by=.5),
     labels=seq(round(mu-3*sigma,0),round(mu+3*sigma,0),by=.5)
     )
...
`r paste('x = c(',paste(x,collapse=','),')',sep='')`

This has sample standard deviation = `r sd(x)`

## A (potentially) helpful applet  {.nostretch .scrollable} 

On exams, there will be a link to an applet:  [Probability applet](https://people.tamu.edu/~scottcrawford/ftable.html){.external target="_blank"}


[Note: This applet is totally optional and unnecessary if you use R]{.emphasis}



:::{.scrolling}
![](figures/appletScott.png){width=700px fig-alt="Standard Normal Table for getting probabilities from Z-scores"}
...
-   [Value:]{.smallCap} We need to get standardized z-score from applet and then unstandardize to get value.
...
[(here, 1.751 came from the applet by using the probability .96 from SAT example)]{.subscript}
:::
