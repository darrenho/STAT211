...
$$
\pre{z-score} = \frac{x - \mu}{\sigma}
$$

The main use case is comparing values from two different distributions

[Question:]{.smallCap} Which of the two values is more [extreme]{.emphasis}?

[Answer:]{.smallCap} Whichever has large [magnitude]{.emphasis} z-score (that is, |z-score|)

[Example:]{.example} We want to compare olympic records in men's sprinting (9.63 seconds) and women's sprinting (10.61 seconds)

Which is more extreme?  Let's look at a table:
...
The z-score for men is `r zScoreMen` while for women it is `zScoreWomen`.  Hence, the women's result is more extreme as:
...
[(This is sometimes known as the [empirical rule]{.definition})]{.subscript}
...
```{r}
outPlot = outPlot +   stat_function(fun = funcShaded, args = list(lower_bound = 2), 
